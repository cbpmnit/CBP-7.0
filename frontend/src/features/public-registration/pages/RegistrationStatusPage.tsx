...
export function RegistrationStatusPage() {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errorMsg = err instanceof Error ? err.message : "Unable to reach the server. Please verify your connection and try again."
      setError(errorMsg)
    } finally {
      setLoading(false)
    }
        ...
        <Card className="p-6 sm:p-8 bg-white rounded-3xl border border-slate-200 shadow-xl space-y-6">
          ...
          {loading && (
            <div className="py-8 flex flex-col items-center justify-center gap-3 text-slate-500">
              <Spinner size="md" />
              <span className="text-xs font-semibold">Verifying registration database...</span>
            </div>
          )}
                  ...
                  <div className="space-y-3 bg-slate-50 p-4 rounded-2xl border border-slate-200 text-xs sm:text-sm">
                    <div className="flex justify-between items-center py-1 border-b border-slate-200">
                      <span className="text-slate-500 font-medium flex items-center gap-1.5">
                        <FiUser className="text-cyan-800" /> Full Name:
                      </span>
                      <span className="font-bold text-slate-900">{result.fullName || "N/A"}</span>
                    </div>
                    ...
                    {result.department && (
                      <div className="flex justify-between items-center py-1 border-b border-slate-200">
                        <span className="text-slate-500 font-medium flex items-center gap-1.5">
                          <FiBookOpen className="text-cyan-800" /> Department:
                        </span>
                    ...
                    {result.programLevel && (
                      <div className="flex justify-between items-center py-1 border-b border-slate-200">
                        <span className="text-slate-500 font-medium flex items-center gap-1.5">
                          <FiAward className="text-cyan-800" /> Program Level:
                        </span>
                        <span className="font-medium text-slate-800">{result.programLevel}</span>
                      </div>
                    )}
                    ...
                    <div className="flex justify-between items-center py-1 border-b border-slate-200">
                      ...
                      <Badge variant={result.paymentStatus === "REGISTERED" || result.paymentStatus === "SUCCESS" ? "success" : "warning"}>
                    ...
                    <div className="flex justify-between items-center py-1 border-b border-slate-200">
                      <span className="text-slate-500 font-medium flex items-center gap-1.5">
                        <FiShield className="text-emerald-800" /> Account Verified:
                      </span>
                      <Badge variant={result.accountVerified ? "success" : "neutral"}>
                ...
                <div className="space-y-4 animate-in fade-in duration-200">
                  <div className="flex items-center gap-3 p-4 bg-amber-50 border border-amber-200 rounded-2xl">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-amber-700">{result.message || "No registration found with provided details."}</p>
                    </div>
                  </div>
