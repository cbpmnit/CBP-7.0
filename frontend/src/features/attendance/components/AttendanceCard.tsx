...
export function AttendanceCard({ summary, loading }: AttendanceCardProps) {
  if (loading) {
    return (
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          <h3 className="text-sm font-bold text-slate-900 flex items-center gap-2">
            <FiPercent className="text-cyan-700 text-base" />
            <span>Attendance Progress</span>
          </h3>
          <StatusBadge
            status={isEligible ? "ACTIVE" : "PENDING"}
            label={isEligible ? "Eligible for Certificate" : "< 75% Requirement"}
          />
        </div>
        ...
        <div className="w-full bg-slate-100 rounded-full h-2 overflow-hidden mb-1 border border-slate-200">
          <div
