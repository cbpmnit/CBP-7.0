...
const DEFAULT_PRESETS: CreateNotificationTemplateRequest[] = [
  {
  ...
  const fetchTemplates = async () => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        // Initialize default templates if list is empty
        setTemplates(
      ...
      // Fallback to presets on initial demo load
      setTemplates(
        DEFAULT_PRESETS.map((p, idx) => ({
          id: `preset-${idx + 1}`,
          templateName: p.templateName,
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between border-b border-slate-200 pb-6 gap-4">
            <div>
              <h1 className="text-3xl font-extrabold tracking-tight text-slate-900">
                Email <span className="gradient-text-cyan">Templates</span>
              </h1>
