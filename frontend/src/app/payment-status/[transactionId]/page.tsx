...
export default function PaymentStatusPage() {
  ...
  const verifyTransaction = async () => {
    ...
    try {
      setStatus("VERIFYING")
      setError(null)
      // Step 1: Call backend PhonePe status verify to trigger callback sync or check status details
      await api.get(`/api/v1/payment/status/${transactionId}`)
      
      // Step 2: Transition to polling payment status
      setStatus("POLLING")
      setPollCount(0)
    } catch (err: any) {
      console.error("Initial verification trigger failed:", err)
      // Fallback directly to polling anyway in case client wants to verify the DB state
      setStatus("POLLING")
      setPollCount(0)
    }
  }
            ...
            <div className="glass-card rounded-3xl p-8 border-cyan-500/30 text-center">
              ...
              {status === "SUCCESS" && (
                <>
                  ...
                  <Link
                    href="/dashboard"
                    className="w-full inline-flex items-center justify-center rounded-xl bg-cyan-500 text-black py-4 text-xs font-extrabold uppercase tracking-widest hover:bg-cyan-400 transition shadow-[0_0_15px_rgba(0,240,255,0.3)] cursor-pointer font-bold"
                  >
                    Return to Dashboard
                  </Link>
                </>
              ...
              {status === "FAILED" && (
                <>
                  ...
                  <div className="flex gap-4">
                    <Link
                      href="/payment"
                      className="flex-1 inline-flex items-center justify-center rounded-xl bg-cyan-500 text-black py-3.5 text-xs font-extrabold uppercase tracking-widest hover:bg-cyan-400 transition cursor-pointer font-bold"
                    >
                      Retry Payment
                    </Link>
                    <Link
                      href="/dashboard"
                      className="flex-1 inline-flex items-center justify-center rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 text-white py-3.5 text-xs font-extrabold uppercase tracking-widest transition cursor-pointer font-bold"
                    >
                      Dashboard
                    </Link>
                  </div>
                ...
                <>
                  ...
                  <div className="flex flex-col sm:flex-row gap-4">
                    <button
                      ...
                      className="flex-1 inline-flex items-center justify-center gap-2 rounded-xl bg-cyan-500 text-black py-3.5 text-xs font-extrabold uppercase tracking-widest hover:bg-cyan-400 transition cursor-pointer font-bold"
                    >
                      <FiRefreshCw />
                      Re-Verify Status
                    </button>
