...
import { FiCheckCircle, FiAward, FiArrowLeft, FiRefreshCw } from "react-icons/fi"
  ...
  useEffect(() => {
    ...
    const fetchStatus = async () => {
      try {
        ...
        setError("Unable to load registration confirmation details.")
      } finally {
        setLoading(false)
      }
    }
  ...
  return (
    <Card className="shadow-xl rounded-3xl border border-slate-200 overflow-hidden bg-white p-6 sm:p-10 max-w-lg mx-auto text-center space-y-6">
      ...
      <div className="pt-2">
        ...
        >
          <FiArrowLeft className="mr-2" /> Return to Public Portal
        </Button>
      </div>
    </Card>
  )
